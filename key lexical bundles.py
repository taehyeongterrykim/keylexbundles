...
# Set up directory paths for target and reference corpora (modify paths to point to your corpus files)
path = 'FolderWithTargetCorpusFiles/*.txt'
path2 = 'FolderWithReferenceCorpusFiles/*.txt'

# Set up directory path for output result file
file_out = open('output.csv', "w+",  encoding='utf-8', errors='replace')
...
#split each sentence into words
        ...
        # Update global dictionary outside the loop. Each key in this dictionary represents a bundle, and the associated value is a list containing the normalized frequencies of that bundle across different texts
        for key in text_bundles_count:
...
# Calculate mean and standard deviation of normalized frequencies of a bundle across texts
...
# If the current word is not in the reference corpus use this formula
            ...
            E_T = word_count * (freq_T + freq_R) / (word_count + word_count2)
            E_R = word_count * (freq_T + freq_R) / (word_count + word_count2)
...
for i in sorted(lexical_bundles, key=lambda x: (text_counts.get(x), lexical_bundles.get(x)), reverse=True):
    if i in lexical_bundles2 and i in text_counts2 and lexical_bundles[i] >= 1 and text_counts[i] >= 1: # Write lexical bundle information to the output file for a bundle that satisfies the frequency and distribution threshold (currently no theshold for both; threshold can be set by changing the number 1 to a desired value)
